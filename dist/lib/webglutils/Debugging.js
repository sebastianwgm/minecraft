...
export class Debugger {
     ...
     */
    static log(msg, fontSize = 12, color = "black") {
     ...
     * @param color the color of the text
     */
    static logError(msg, fontSize = 12, color = "red") {
     ...
     */
    static defaultErrorCallback(error, functionName, args) {
     ...
     * @param args the arguments to the function
     */
    static throwOnError(error, functionName, args) {
        var argStr = "";
        var numArgs = args.length;
        for (var ii = 0; ii < numArgs; ++ii) {
            argStr += ((ii == 0) ? '' : ', ') +
     ...
     * @param args the arguments to the function
     */
    static logFunctionCall(functionName, args) {
        var argStr = "";
        var numArgs = args.length;
        for (var ii = 0; ii < numArgs; ++ii) {
            argStr += ((ii == 0) ? '' : ', ') +
     ...
     * @param args the arguments to the function
     */
    static logErrorOnUndefinedArg(functionName, args) {
     ...
     * @param args the arguments to the function
     */
    static throwErrorOnUndefinedArg(functionName, args) {
        for (var ii = 0; ii < args.length; ++ii) {
            if (args[ii] === undefined) {
     ...
     * @param args the arguments to the function
     */
    static logAndValidate(functionName, args) {
     ...
     * @param glCallback
     */
    static generateGLCallback(glFunctionName, glCallback) {
     ...
     */
    static generateGLErrorCallback(glFunctionName, glErrorCallback) {
        return function (error, functionName, args) {
            if (functionName === glFunctionName) {
     ...
     * @param glCallbacks
     */
    static generateGLCallbackFromList(glCallbacks) {
        return function (functionName, args) {
     ...
     */
    static generateGLErrorCallbackFromList(glErrorCallbacks) {
        return function (error, functionName, args) {
    ...
    static init(ctx) {
        if (Debugger.glEnums == null) {
            ...
            for (var propertyName in ctx) {
                var temp = ctx[propertyName];
                if (typeof temp == 'number') {
                    Debugger.glEnums[temp] = propertyName;
                    Debugger.enumStringToValue[propertyName] = temp;
                }
            }
        }
    }
     ...
     */
    static mightBeEnum(value) {
        Debugger.checkInit();
        return (Debugger.glEnums[value] !== undefined);
    }
    /**
     * PROVIDED BY KHRONOS:
     ...
     */
    static glEnumToString(value) {
        Debugger.checkInit();
        var name = Debugger.glEnums[value];
     ...
     */
    static glFunctionArgToString(functionName, numArgs, argumentIndex, value) {
        var funcInfo = Debugger.glValidEnumContexts[functionName];
                ...
                if (funcInfo[argumentIndex]) {
                    if (typeof funcInfo[argumentIndex] === 'object' &&
                        ...
                        for (var i = 0; i < enums.length; ++i) {
                            var enumValue = Debugger.enumStringToValue[enums[i]];
     ...
     */
    static glFunctionArgsToString(functionName, args) {
        // apparently we can't do args.join(",");
        var argStr = "";
        var numArgs = args.length;
        for (var ii = 0; ii < numArgs; ++ii) {
     ...
     * @param propertyName
     */
    static makePropertyWrapper(wrapper, original, propertyName) {
        //log("wrap prop: " + propertyName);
        Object.defineProperty(wrapper, propertyName, {
            get: function () { return original[propertyName]; },
            set: function (value) { original[propertyName] = value; }
        });
    }
    /**
     * PROVIDED BY KHRONOS:
     ...
     */
    static makeDebugContext(ctx, opt_onErrorFunc = Debugger.defaultErrorCallback, opt_onFunc) {
        Debugger.init(ctx);
